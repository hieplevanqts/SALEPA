  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl max-w-2xl w-full shadow-2xl max-h-[90vh] overflow-auto">
        {/* Header */}
        <div className="sticky top-0 flex justify-between items-center p-6 border-b border-gray-200 bg-gradient-to-r from-orange-600 via-orange-500 to-purple-600 rounded-t-2xl print:hidden">
          <div>
                ...
                <div key={index} className="text-sm">
                  <div className="grid grid-cols-12 gap-2">
                    ...
                    <div className="col-span-2 text-right text-gray-700">
                      {((item.variantPrice !== undefined ? item.variantPrice : item.price) || 0).toLocaleString('vi-VN')}đ
                    </div>
                    <div className="col-span-3 text-right font-semibold text-gray-900">
                      {((((item.variantPrice !== undefined ? item.variantPrice : item.price) || 0) * item.quantity) - (item.discount || 0)).toLocaleString('vi-VN')}đ
                    </div>
                  </div>
                </div>
              ))}
