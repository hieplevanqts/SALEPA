...
export function OrderDetailFullScreen({ order, onClose, onCollectPayment, onPrintReceipt, onEdit, onDelete }: OrderDetailFullScreenProps) {
  ...
  return (
    <div className="h-full bg-gray-50 flex flex-col">
      {/* Main Content - Full width of content area (not covering sidebar) */}
                ...
                <>
                  ...
                  <div className="bg-white rounded-2xl border-2 border-gray-200 p-6">
                    <h3 className="text-base font-bold text-gray-900 mb-4 flex items-center gap-2">
                      ...
                      {t('orderItems') || 'Danh sách sản phẩm/dịch vụ'}
                    </h3>
                            ...
                            <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                              <td className="py-3 px-4 text-base text-gray-900">
                                <div>
                                  ...
                                  {/* Display variant information inline */}
                                  {(item.variantTitle || item.variantCode) && (
                                    <div className="flex items-center gap-2 flex-wrap">
                                      {item.variantTitle && (
                                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium" style={{ backgroundColor: '#FFF7ED', color: '#FE7410' }}>
                                          {item.variantTitle}
                                        </span>
                                      )}
                                      {item.variantCode && (
                              ...
                              <td className="py-3 px-4 text-base text-right text-gray-900">
                                {((item.variantPrice !== undefined ? item.variantPrice : item.price) || 0).toLocaleString()}đ
                              </td>
                              <td className="py-3 px-4 text-base text-right font-semibold text-gray-900">
                                {(((item.variantPrice !== undefined ? item.variantPrice : item.price) || 0) * item.quantity).toLocaleString()}đ
                              </td>
                            </tr>
                          ))}
                        </tbody>
