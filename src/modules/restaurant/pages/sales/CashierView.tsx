...
export function CashierView() {
  ...
  const [currentScreen, setCurrentScreen] = useState<CashierScreen>('orders');
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false);

  const menuItems = [
    { id: 'orders' as CashierScreen, icon: Receipt, label: t.ordersAndPayment || t.orders, color: 'blue' },
    { id: 'shift' as CashierScreen, icon: Clock, label: t.myShift, color: 'green' },
  ];

  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      {/* Sidebar */}
      <div className={`${sidebarCollapsed ? 'w-20' : 'w-64'} bg-gradient-to-b from-green-600 to-green-800 text-white flex flex-col transition-all duration-300`}>
        {/* Header */}
        <div className="p-4 border-b border-green-700">
          {!sidebarCollapsed ? (
            <>
              <div className="flex items-center justify-between mb-2">
                <h1 className="text-xl font-bold">💰 {t.cashier}</h1>
                <button
              ...
              <div className="flex items-center gap-2 bg-green-700 rounded-lg px-3 py-2">
                ...
                <div>
                  <p className="text-sm font-semibold">{t.cashier}</p>
                  <p className="text-xs text-green-200">{t.staff}</p>
                </div>
              </div>
            </>
          ) : (
        ...
        <nav className="flex-1 p-3 space-y-2">
          ...
          >
            <Home className={`w-6 h-6 ${sidebarCollapsed ? 'mx-auto' : ''}`} />
            {!sidebarCollapsed && (
              <span className="font-semibold text-sm">{t.backToHome || t.home}</span>
            )}
          </a>
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`w-6 h-6 ${sidebarCollapsed ? 'mx-auto' : ''}`} />
                {!sidebarCollapsed && (
        ...
        <div className="p-3 border-t border-green-700">
          {!sidebarCollapsed ? (
            ...
            >
              <span className="text-2xl">{language === 'vi' ? '🇻🇳' : '🇺🇸'}</span>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold text-gray-900">
                {currentScreen === 'orders' && (t.ordersAndPayment || t.orders)}
                {currentScreen === 'shift' && t.myShift}
              </h2>
              <p className="text-sm text-gray-500">
                {currentScreen === 'orders' && (t.viewOrdersAndCollectPayment || 'View orders and collect payment')}
                {currentScreen === 'shift' && (t.manageYourShift || 'Manage your work shift')}
              </p>
            </div>
            ...
            <div className="flex items-center gap-4">
              <div className="bg-green-50 border-2 border-green-200 rounded-xl px-4 py-2">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-xs text-green-600 font-medium">{t.todayRevenue}</p>
                    <p className="text-lg font-bold text-green-700">
                      {(JSON.parse(localStorage.getItem('pos-orders') || '[]') as any[])
        ...
        <div className="flex-1 overflow-hidden">
          {currentScreen === 'orders' && <CashierOrderHistory />}
