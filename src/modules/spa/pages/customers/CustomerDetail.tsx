            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => {
                      import('../../../../lib/spa-lib/demoPackagesV2').then(module => {
                                            ...
                                            <div className="bg-blue-50 rounded-lg p-4 space-y-2">
                                              ...
                                              {appointment.code && (
                                                <div className="flex items-center gap-2 text-sm">
                                                  <span className="font-medium text-gray-700 w-24">Mã lịch hẹn:</span>
                                                  <span className="text-gray-900 font-semibold">{appointment.code}</span>
                                                </div>
                                              )}
                                              <div className="flex items-center gap-2 text-sm">
                                              ...
                                              {/* Technicians - Updated to use technicianNames from services */}
                                              {appointment.services
                                                .filter(s => s.sessionNumber === sessionNum)
                                                .map((service, idx) => {
                                                  // Use technicianNames if available, otherwise fallback to technicianId
                                                  const techNames = service.technicianNames && service.technicianNames.length > 0
                                                    ? service.technicianNames.join(', ')
                                                    : null;
                                                  
                                                  if (!techNames && !service.technicianId) return null;
                                                  
                                                  const displayName = techNames || users.find(u => u.id === service.technicianId)?.fullName;
                                                  
                                                  return displayName ? (
                                                    <div key={idx} className="flex items-center gap-2 text-sm">
                                                      <span className="font-medium text-gray-700 w-24">Kỹ thuật viên:</span>
