...
export function Settings() {
  ...
  const [activeSection, setActiveSection] = useState<'general' | 'store' | 'data' | 'notifications' | 'appearance' | 'advanced' | 'payment' | 'invoice'>('general');
  ...
  const loadInvoiceSettings = () => {
    const savedInvoiceSettings = localStorage.getItem('pos-invoice-settings');
    if (savedInvoiceSettings) {
      try {
        return JSON.parse(savedInvoiceSettings);
      } catch (e) {
        return {};
      }
    }
  ...
  const initialInvoiceSettings = loadInvoiceSettings();

  // Invoice settings (HĐDT - Hóa đơn điện tử)
  const [invoiceTaxCode, setInvoiceTaxCode] = useState(initialInvoiceSettings.taxCode || '');
  const [invoiceUsername, setInvoiceUsername] = useState(initialInvoiceSettings.username || '');
  const [invoicePassword, setInvoicePassword] = useState(initialInvoiceSettings.password || '');
  const [showInvoicePassword, setShowInvoicePassword] = useState(false);
  ...
  const handleSaveSettings = () => {
    ...
    // Save invoice settings separately
    const invoiceSettings = {
      taxCode: invoiceTaxCode,
      username: invoiceUsername,
      password: invoicePassword,
    };
    localStorage.setItem('pos-invoice-settings', JSON.stringify(invoiceSettings));
  ...
  const menuItems = [
    ...
    { id: 'invoice' as const, icon: FileText, label: t('invoiceSettings') },
  ];
          ...
          {activeSection === 'invoice' && (
            <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h3 className="text-gray-900 font-bold">{t('invoiceSettings')}</h3>
                  <p className="text-sm text-gray-500">{t('invoiceConfiguration')}</p>
                </div>
              </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setInvoiceUsername(e.target.value)}
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        type={showInvoicePassword ? 'text' : 'password'}
                      ...
                      <button
                        ...
                        onClick={() => setShowInvoicePassword(!showInvoicePassword)}
                      >
                        {showInvoicePassword ? (
