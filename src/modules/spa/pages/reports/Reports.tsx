...
import { usePagination } from '../../../../lib/spa-lib/usePagination';
    ...
    try {
      ...
      switch (activeTab) {
        case 'revenue-overview':
          exportData = demoRevenueOverview.map(item => ({
            'Ngày': item.period,
            'Doanh thu': item.revenue,
            'Số hóa đơn': item.orders,
            'Giá trị TB/hóa đơn': Math.round(item.revenue / item.orders)
          }));
          sheetName = 'DT Tổng hợp';
          fileName = `BaoCao_DoanhThuTongHop_${new Date().toISOString().split('T')[0]}.xlsx`;
          break;
        ...
        case 'revenue-staff':
          ...
          exportData = filteredStaff.map((staff: any) => ({
            'Nhân viên': staff.name,
            'DT Dịch vụ': staff.serviceRevenue,
            'DT Gói liệu trình': staff.packageRevenue,
            'DT Sản phẩm': staff.productRevenue,
            'Tổng doanh thu': staff.totalRevenue
          }));
          sheetName = 'DT Nhân viên';
          fileName = `BaoCao_DoanhThuNhanVien_${new Date().toISOString().split('T')[0]}.xlsx`;
          break;

        case 'revenue-service':
          exportData = demoServiceRevenue.map((service: any) => {
            const topStaff = Object.entries(service.staff).sort((a: any, b: any) => b[1] - a[1])[0];
            return {
              'Dịch vụ': service.name,
              'Doanh thu': service.revenue,
              'Số lượt': service.count,
              'NV thực hiện nhiều nhất': topStaff ? `${topStaff[0]} (${topStaff[1]} lượt)` : '-'
            };
          });
          sheetName = 'DT Dịch vụ';
          fileName = `BaoCao_DoanhThuDichVu_${new Date().toISOString().split('T')[0]}.xlsx`;
          break;

        case 'revenue-package':
          exportData = demoPackageRevenue.map((pkg: any) => {
            const topStaff = Object.entries(pkg.staff).sort((a: any, b: any) => b[1] - a[1])[0];
            return {
              'Gói liệu trình': pkg.name,
              'Số gói': pkg.count,
              'Giá trị': pkg.revenue,
              'NV bán hàng': topStaff ? `${topStaff[0]} (${topStaff[1]} gói)` : '-'
            };
          });
          sheetName = 'DT Gói';
          fileName = `BaoCao_DoanhThuGoi_${new Date().toISOString().split('T')[0]}.xlsx`;
          break;

        case 'revenue-product':
          exportData = demoProductRevenue.map((product: any) => {
            ...
            return {
              ...
              'NV bán nhiều nhất': topStaff ? `${topStaff[0]} (${topStaff[1]} SP)` : '-'
            };
          });
          sheetName = 'DT Sản phẩm';
          fileName = `BaoCao_DoanhThuSanPham_${new Date().toISOString().split('T')[0]}.xlsx`;
          break;

        case 'customer-report':
          exportData = demoCustomerData.map(customer => ({
            'Khách hàng': customer.name,
            'Số điện thoại': customer.phone,
            'Số lần mua': customer.orderCount,
            'Tổng chi tiêu': customer.totalSpent
          }));
          sheetName = 'Báo cáo KH';
          fileName = `BaoCao_KhachHang_${new Date().toISOString().split('T')[0]}.xlsx`;
          break;

        case 'appointment-report':
          exportData = demoAppointmentStaffData.map(staff => ({
            'Kỹ thuật viên': staff.name,
            'Tổng lịch hẹn': staff.totalAppointments,
            'Hoàn thành': staff.completed,
            'Hủy': staff.cancelled,
            'Tỷ lệ hoàn thành (%)': staff.completionRate
          }));
          sheetName = 'Lịch hẹn';
          fileName = `BaoCao_LichHen_${new Date().toISOString().split('T')[0]}.xlsx`;
          break;

        case 'inventory-report':
          const productMetrics = demoInventoryData.map(product => ({
            'Sản phẩm': product.name,
            'Tồn đầu kỳ': product.stock,
            'Đã bán': product.sold,
            'Tồn cuối kỳ': product.stock - product.sold,
            'Giá trị tồn': (product.stock - product.sold) * product.price
          }));
          exportData = productMetrics;
          sheetName = 'Tồn kho';
          fileName = `BaoCao_TonKho_${new Date().toISOString().split('T')[0]}.xlsx`;
          break;

        default:
          toast.error('❌ Không hỗ trợ xuất báo cáo này!');
          return;
      }
      ...
      toast.success(`✅ Đã xuất ${exportData.length} dòng dữ liệu ra Excel!`, {
        duration: 3000,
      });
    } catch (error) {
          ...
          <table className="modern-table">
            <thead>
              <tr>
                ...
                <th className="table-header text-right">DT Dịch vụ</th>
                <th className="table-header text-right">DT Gói</th>
