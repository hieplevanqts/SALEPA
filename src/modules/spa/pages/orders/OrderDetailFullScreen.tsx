                ...
                <>
                  ...
                  <div className="bg-white rounded-2xl border-2 border-gray-200 p-6">
                    <h3 className="text-base font-bold text-gray-900 mb-4 flex items-center gap-2">
                      ...
                      {t('orderItems') || 'Danh sch sản phẩm/dịch vụ/liệu trình'}
                    </h3>
                      ...
                      <table className="w-full">
                        ...
                        <tfoot>
                          ...
                          {order.voucherCode && order.voucherDiscount > 0 ? (
                            <tr className="border-b border-gray-100">
                              <td colSpan={3} className="py-3 px-4 text-right font-semibold text-gray-700 text-base">
                                {t('voucher') || 'Voucher'} ({order.voucherCode}):
                              </td>
                              <td className="py-3 px-4 text-right font-semibold text-green-600 text-base">
                                -{order.voucherDiscount.toLocaleString()}đ
                              </td>
                            </tr>
                          ) : null}
                  ...
                  {order.paymentHistory && order.paymentHistory.length > 0 ? (
                    <div className="space-y-4">
                      ...
                      {[...order.paymentHistory].reverse().map((payment: any, index: number) => {
                        ...
                        const paymentCode = `TT${datePrefix}${String(index + 1).padStart(4, '0')}`;
                        ...
                        const remainingAfterPayment = order.total - (previousPaymentsTotal + payment.amount);
                        ...
                        const shouldShowRemaining = !wasCapped && remainingAfterPayment > 0; // Fixed: show if still owe money (positive remaining)
                              ...
                              {shouldShowChange ? (
                                ...
                                <div className="bg-red-50 rounded-lg p-3">
                                  ...
                                  <div className="font-semibold text-red-600">
                                    {remainingAfterPayment.toLocaleString()}đ
                                  </div>
                                </div>
                              ) : (
            ...
            <div className="lg:col-span-1 space-y-6">
              ...
              <div className="bg-white rounded-2xl border-2 border-gray-200 p-6 lg:sticky lg:top-6">
                <h3 className="text-base font-bold text-gray-900 mb-4">
                  {t('orderSummary') || 'Tổng quan hóa đơn'}
                </h3>
                
                {/* Order ID */}
                <div className="bg-gray-50 rounded-xl p-4 mb-4">
                  <div className="text-sm text-gray-600 mb-1">{t('orderNumber') || 'Mã hóa đơn'}</div>
                  <div className="font-bold text-gray-900 text-lg">{order.id.slice(-8).toUpperCase()}</div>
                </div>
                ...
                {/* Ẩn phần giảm giá thủ công - không cần thiết */}
                {false && order.discount > 0 && (
