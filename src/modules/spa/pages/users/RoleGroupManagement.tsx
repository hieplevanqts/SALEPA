...
import { systemPermissions, defaultRoleGroups } from '../../../../lib/spa-lib/permissionData';
...
export function RoleGroupManagement() {
  ...
  const isCategoryFullySelected = (category: string) => {
    const categoryPerms = permissionsByCategory[category];
    const categoryPermIds = categoryPerms.map(p => p.id);
    return categoryPermIds.length > 0 && categoryPermIds.every(id => formData.permissions.includes(id));
  };
  ...
  const getCategoryName = (category: string) => {
    const names: Record<string, { vi: string; en: string }> = {
      ...
      stock: { vi: 'Quản lý kho', en: 'Stock Management' },
      categories: { vi: 'Danh mục', en: 'Categories' },
      account: { vi: 'Quản lý tài khoản', en: 'Account Management' },
            ...
            <div>
              ...
              <div className="border rounded-lg p-4 space-y-4 max-h-[50vh] overflow-y-auto">
                {Object.entries(permissionsByCategory).map(([category, perms]) => (
                  <div key={category} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-500">
                          ({perms.length} {language === 'vi' ? 'quyền' : 'permissions'})
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <label
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 ml-2">
                      {perms.map((perm) => (
                        <div key={perm.id} className="flex items-center space-x-2 bg-white p-3 rounded border border-gray-100 hover:border-[#FE7410] transition-colors">
                          <Checkbox
                            id={`perm-${perm.id}`}
              ...
              <div className="flex items-center justify-between mt-3">
                ...
                <p className="text-xs text-gray-400">
                  {language === 'vi'
                    ? `Tổng ${systemPermissions.length} quyền có sẵn`
                    : `Total ${systemPermissions.length} permissions available`}
                </p>
              </div>
            </div>
          </div>
